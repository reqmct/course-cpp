...
public:
    ...
    std::ostream &print(std::ostream &strm) const {
        return strm << "Expected: " << std::hex << static_cast<unsigned>(m_expected) << ", found: " << std::hex
                    << static_cast<unsigned>(m_found) << " at " << std::dec << m_mismatch_index;
    }

    friend std::ostream &operator<<(std::ostream &strm, const CheckResult &cr) { return cr.print(strm); }

    static CheckResult success(const unsigned char expected) { return expected; }
    static CheckResult failure(const std::size_t mismatch_index, const unsigned char expected,
...
private:
    CheckResult(const unsigned char expected_)
        : m_success(true), m_mismatch_index(-1), m_expected(expected_), m_found(expected_) {}

    CheckResult(const std::size_t mismatch_index_, const unsigned char expected_, const unsigned char found_)
        : m_success(false), m_mismatch_index(mismatch_index_), m_expected(expected_), m_found(found_) {}
...
struct Dummy {
    ...
    static void reset() { population = 0; }
...
#define DUMMY(size, marker_value)         \
    ...
    template <>                           \
    unsigned char Dummy<size>::marker() { \
        return marker_value;              \
    }
...
struct DifferentSizesTest<L<Ts...>>: ::testing::Test {
    ...
    static void reset() { (Ts::reset(), ...); }

    static bool balanced() { return ((Ts::population == 0) && ... && true); }
    ...
    DifferentSizesTest() : alloc(N, {sizeof(Ts)...}), sizes({sizeof(Ts)...}) {}
    ...
    void destroy(const std::size_t i, const void *ptr) { destroy_impl(i, Indexes{}, ptr); }

    template <std::size_t I, class... Args>
    auto create(Args &&...args) -> std::add_pointer_t<std::tuple_element_t<I, L<Ts...>>> {
        using T = std::tuple_element_t<I, L<Ts...>>;
    ...
    auto check(const std::size_t i, const void *ptr) { return check_impl(i, Indexes{}, ptr); }
};

template <class... Ts>
using L           = std::tuple<Ts...>;
using TestedTypes = ::testing::Types<L<D1>, L<D1, D2>, L<D2, D4>, L<D1, D2, D5>, L<D1, D2, D4, D10>,
                                     L<D2, D4, D5, D6, D10>, L<D2, D4, D10, D20>, L<D4, D10, D20, D256>>;
...
TYPED_TEST(DifferentSizesTest, out_of_memory) {
    ...
    for (std::size_t i = 0; i < this->SizeCount; ++i) {
        ...
        EXPECT_THROW({ this->create(i); }, std::bad_alloc);
    }
    std::size_t k = 0;
    for (std::size_t i = 0; i < this->SizeCount; ++i) {
        for (std::size_t j = 0; j < this->Capacities[i]; ++j) {
