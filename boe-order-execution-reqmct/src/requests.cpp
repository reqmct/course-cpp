...
#include "encode_opt_fields.h"
#include "fill_fields.h"
...
{
    ExecutionDetails exec_details = fill_order_execution(message);
    return exec_details;
}

RestatementDetails decode_order_restatement(const std::vector<unsigned char> & message)
{
    RestatementDetails restatement_details = fill_order_details(message);
...
std::vector<unsigned char> request_optional_fields_for_message(const ResponseType response)
{
    ...
    case ResponseType::OrderExecution:
        return get_optional_fields_order_execution();
    case ResponseType::OrderRestatement:
        return get_optional_fields_order_restatement();
    }
    return {};
}