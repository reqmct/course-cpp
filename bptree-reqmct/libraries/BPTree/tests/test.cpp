...
struct BigOne {
    ...
    operator int() const { return std::stoi(std::string(data.begin(), data.end())); }

    friend std::ostream& operator<<(std::ostream& strm, const BigOne& x) { return strm << static_cast<int>(x); }

    friend bool operator==(const BigOne& lhs, const BigOne& rhs) { return lhs.data == rhs.data; }

    // friend bool operator!=(const BigOne& lhs, const BigOne& rhs) { return !(lhs == rhs); }
};

template <class T>
struct IntCompat {
    static T create(const int x) { return x; }
    static int value(const T& x) { return x; }
    static bool empty(const T& x) { return x == T{}; }
};
...
struct IntCompat<std::string> {
    ...
    static int value(const std::string& x) { return std::stoi(x); }
    static bool empty(const std::string& x) { return x.empty(); }
};
...
struct Type {
    ...
    static std::pair<Key, Value> create(const int x) { return {create_key(x), create_value(x)}; }
    static Key create_key(const int x) { return IntCompat<Key>::create(x); }
    static Value create_value(const int x) { return IntCompat<Value>::create(x); }

    static int key(const Key& x) { return IntCompat<Key>::value(x); }
    static int key(const std::pair<const Key, Value>& x) { return IntCompat<Key>::value(x.first); }
    static int value(const std::pair<const Key, Value>& x) { return IntCompat<Value>::value(x.second); }
    static int value(const Value& x) { return IntCompat<Value>::value(x); }

    static bool empty_value(const Value& v) { return IntCompat<Value>::empty(v); }
};
...
struct BPTreeTest: ::testing::Test {
    ...
    const Tree& const_tree() { return tree; }

    void insert(const std::pair<typename T::key_type, typename T::value_type>& el) { tree.insert(el.first, el.second); }
...
using TestedTypes =
    ::testing::Types<Type<int, std::string>, Type<std::string, int>, Type<std::string, std::string>, Type<int, BigOne>>;
...
TYPED_TEST(BPTreeTest, at) {
    EXPECT_THROW({ this->tree.at(TypeParam::create_key(7)); }, std::out_of_range);
    this->insert(TypeParam::create(7));
    ...
    EXPECT_TRUE(std::is_lvalue_reference_v<decltype(this->tree.at(key))>)
        << "'at' method should return an lvalue reference";
    EXPECT_FALSE(std::is_const_v<std::remove_reference_t<decltype(this->tree.at(key))>>)
        << "non-const 'at' method should return a non-const reference";
    EXPECT_TRUE(std::is_const_v<std::remove_reference_t<decltype(this->const_tree().at(key))>>)
        << "const 'at' method should return a const reference";
}

TYPED_TEST(BPTreeTest, index) {
    EXPECT_TRUE(std::is_lvalue_reference_v<decltype(this->tree.operator[](TypeParam::create_key(111)))>)
        << "operator [] should return an lvalue reference";
...
TYPED_TEST(BPTreeTest, erase_range) {
    ...
    const auto it       = this->tree.erase(this->const_tree().find(TypeParam::create_key(51)),
                                           this->const_tree().find(TypeParam::create_key(91)));
...
TYPED_TEST(BPTreeTest, several) {
    ...
    for (int i = 0; i < max; ++i) {
        ...
        EXPECT_EQ(it, this->tree.lower_bound(TypeParam::create_key(i)))
            << "Not found correct lower bound for key " << i;
        ++it;
        EXPECT_EQ(it, this->tree.upper_bound(TypeParam::create_key(i)))
            << "Not found correct upper bound for key " << i;
    ...
    for (int i = -max; i < 0; ++i) {
        ...
        EXPECT_EQ(this->tree.begin(), this->tree.lower_bound(TypeParam::create_key(i)))
            << "Not found correct lower bound for key " << i;
        EXPECT_EQ(this->tree.begin(), this->tree.upper_bound(TypeParam::create_key(i)))
            << "Not found correct upper bound for key " << i;
    }
    for (int i = max; i < 2 * max; ++i) {
        EXPECT_EQ(this->tree.end(), this->tree.find(TypeParam::create_key(i))) << "Found non-existing key " << i;
        EXPECT_EQ(0, this->tree.count(TypeParam::create_key(i))) << "Invalid number of entries for key " << i;
        EXPECT_EQ(this->tree.end(), this->tree.lower_bound(TypeParam::create_key(i)))
            << "Not found correct lower bound for key " << i;
        EXPECT_EQ(this->tree.end(), this->tree.upper_bound(TypeParam::create_key(i)))
            << "Not found correct upper bound for key " << i;
    }
}
...
TYPED_TEST(BPTreeTest, unsorted_insert) {
    const auto elements = {TypeParam::create(111), TypeParam::create(-1), TypeParam::create(0),
                           TypeParam::create(31),  TypeParam::create(7),  TypeParam::create(11),
                           TypeParam::create(17),  TypeParam::create(97), TypeParam::create(1001),
                           TypeParam::create(-59), TypeParam::create(23)};
...
TYPED_TEST(BPTreeTest, many) {
    ...
    for (int i = 0; i < max; ++i) {
        EXPECT_NE(this->tree.end(), this->tree.find(TypeParam::create_key(i))) << "Not found key " << i;
        EXPECT_EQ(1, this->tree.count(TypeParam::create_key(i))) << "Invalid number of entries for key " << i;
        EXPECT_EQ(i, TypeParam::key(*this->tree.find(TypeParam::create_key(i))))
            << "Not found correct key value for " << i;
        auto it = this->tree.find(TypeParam::create_key(i));
        EXPECT_EQ(it, this->tree.lower_bound(TypeParam::create_key(i)))
            << "Not found correct lower bound for key " << i;
        ++it;
