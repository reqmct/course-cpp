...
public:
    Key() : Key(-13131313) {}
    explicit Key(const int n) : super(n) {}

    std::ostream &print(std::ostream &strm) const { return strm << std::string_view{this->data, Size}; }

    friend bool operator<(const Key &lhs, const Key &rhs) { return std::memcmp(lhs.data, rhs.data, Size) < 0; }
...
public:
    ...
    explicit Value(const int n) : super(n) {}

    std::ostream &print(std::ostream &strm) const { return strm << std::string_view{this->data, Size}; }

    friend bool operator<(const Value &lhs, const Value &rhs) { return std::memcmp(lhs.data, rhs.data, Size) < 0; }

    friend std::ostream &operator<<(std::ostream &strm, const Value &value) { return value.print(strm); }
};
...
public:
    ...
    explicit NonCopyableValue(const int n) : super(n) {}
    ...
    std::ostream &print(std::ostream &strm) const { return strm << std::string_view{data, Size}; }

    // friend std::ostream &operator<<(std::ostream &strm, const NonCopyableValue &value) { return value.print(strm); }
};
...
struct BPTree_3: ::testing::Test {
    ...
    static K create_key(const int n) { return K{n}; }

    static V create_value(const int n) { return V{n}; }

    inline static const V s_empty_value;

    static int cmp(const K &lhs, const K &rhs) { return std::memcmp(lhs.data, rhs.data, lhs.size()); }

    static int cmp(const V &lhs, const V &rhs) { return std::memcmp(lhs.data, rhs.data, lhs.size()); }
};
...
struct BPTree_3_descending: ::testing::Test {
    ...
    struct Cmp {
        bool operator()(const K &lhs, const K &rhs) const { return rhs < lhs; }
    };
...
TEST_F(BPTree_3, fill_three_levels__index_operator) {
    ...
    const auto [c_end, t_end] =
        std::mismatch(contents.begin(), contents.end(), tree.begin(), tree.end(), [](const auto &lhs, const auto &rhs) {
            return 0 == cmp(lhs.first, rhs.first) && 0 == cmp(lhs.second, rhs.second);
        });
    EXPECT_EQ(contents.end(), c_end) << "difference at " << std::distance(contents.begin(), c_end) << " with <"
                                     << c_end->first << ", " << c_end->second << ">";
    EXPECT_EQ(tree.end(), t_end) << "difference at " << std::distance(tree.begin(), t_end) << " with <" << t_end->first
                                 << ", " << t_end->second << ">";
}
...
TEST_F(BPTree_3, copy) {
    ...
    std::sort(contents.begin(), contents.end());
    {
        const auto [c_end, t_end] = std::mismatch(
            contents.begin(), contents.end(), tree.begin(), tree.end(), [](const auto &lhs, const auto &rhs) {
                return 0 == cmp(lhs.first, rhs.first) && 0 == cmp(lhs.second, rhs.second);
            });
        EXPECT_EQ(contents.end(), c_end) << "difference at " << std::distance(contents.begin(), c_end) << " with <"
                                         << c_end->first << ", " << c_end->second << ">";
        EXPECT_EQ(tree.end(), t_end) << "difference at " << std::distance(tree.begin(), t_end) << " with <"
                                     << t_end->first << ", " << t_end->second << ">";
    }

    auto copy = tree;
    {
        const auto [c_end, t_end] =
            std::mismatch(copy.begin(), copy.end(), tree.begin(), tree.end(), [](const auto &lhs, const auto &rhs) {
                return 0 == cmp(lhs.first, rhs.first) && 0 == cmp(lhs.second, rhs.second);
            });
        EXPECT_EQ(copy.end(), c_end) << "difference at " << std::distance(copy.begin(), c_end) << " with <"
                                     << c_end->first << ", " << c_end->second << ">";
        EXPECT_EQ(tree.end(), t_end) << "difference at " << std::distance(tree.begin(), t_end) << " with <"
                                     << t_end->first << ", " << t_end->second << ">";
    }
    ...
    {
        const auto [c_end, t_end] = std::mismatch(
            contents.begin(), contents.end(), copy.begin(), copy.end(), [](const auto &lhs, const auto &rhs) {
                return 0 == cmp(lhs.first, rhs.first) && 0 == cmp(lhs.second, rhs.second);
            });
        EXPECT_EQ(contents.end(), c_end) << "difference at " << std::distance(contents.begin(), c_end) << " with <"
                                         << c_end->first << ", " << c_end->second << ">";
        EXPECT_EQ(copy.end(), t_end) << "difference at " << std::distance(copy.begin(), t_end) << " with <"
                                     << t_end->first << ", " << t_end->second << ">";
    }
    {
        const auto [c_end, t_end] = std::mismatch(
            new_contents.begin(), new_contents.end(), tree.begin(), tree.end(), [](const auto &lhs, const auto &rhs) {
                return 0 == cmp(lhs.first, rhs.first) && 0 == cmp(lhs.second, rhs.second);
            });
        EXPECT_EQ(new_contents.end(), c_end) << "difference at " << std::distance(new_contents.begin(), c_end)
                                             << " with <" << c_end->first << ", " << c_end->second << ">";
        EXPECT_EQ(tree.end(), t_end) << "difference at " << std::distance(tree.begin(), t_end) << " with <"
                                     << t_end->first << ", " << t_end->second << ">";
    }
}
...
TEST_F(BPTree_3, move) {
    ...
    {
        const auto [c_end, t_end] = std::mismatch(
            contents.begin(), contents.end(), move.begin(), move.end(), [](const auto &lhs, const auto &rhs) {
                return 0 == cmp(lhs.first, rhs.first) && 0 == cmp(lhs.second, rhs.second);
            });
        EXPECT_EQ(contents.end(), c_end) << "difference at " << std::distance(contents.begin(), c_end) << " with <"
                                         << c_end->first << ", " << c_end->second << ">";
        EXPECT_EQ(move.end(), t_end) << "difference at " << std::distance(move.begin(), t_end) << " with <"
                                     << t_end->first << ", " << t_end->second << ">";
    }

    tree = {std::make_pair(create_key(111), create_value(-111)), std::make_pair(create_key(333), create_value(-333)),
            std::make_pair(create_key(777), create_value(-777))};
    {
        const auto [c_end, t_end] = std::mismatch(
            contents.begin(), contents.end(), move.begin(), move.end(), [](const auto &lhs, const auto &rhs) {
                return 0 == cmp(lhs.first, rhs.first) && 0 == cmp(lhs.second, rhs.second);
...
TEST_F(BPTree_3, erase) {
    ...
    const auto remove = [&](const int n) {
        contents.erase(std::remove_if(contents.begin(), contents.end(),
                                      [x = create_key(n)](const auto &v) { return 0 == cmp(x, v.first); }),
                       contents.end());
...
TEST_F(BPTree_3_descending, small_range) {
    ...
    std::sort(contents.begin(), contents.end(),
              [cmp = Cmp{}](const auto &lhs, const auto &rhs) { return cmp(lhs.first, rhs.first); });
    {
        const auto [c_end, t_end] = std::mismatch(
            contents.begin(), contents.end(), tree.begin(), tree.end(), [](const auto &lhs, const auto &rhs) {
                return 0 == cmp(lhs.first, rhs.first) && 0 == cmp(lhs.second, rhs.second);
